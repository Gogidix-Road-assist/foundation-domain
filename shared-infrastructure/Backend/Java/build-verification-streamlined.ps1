...
foreach ($service in $services) {
    ...
    if ($compileExitCode -eq 0) {
        ...
        Write-Host "    ❌ COMPILE FAILED (${compileDuration}s)" -ForegroundColor Red
        $report += "- ❌ **Compile**: FAILED (${compileDuration}s)`n`n"
        $compileFailed++
        $failedServices += $service
        Pop-Location
        continue
    ...
    if ($jarFile) {
        ...
        Write-Host "    ❌ JAR BUILD FAILED (${jarDuration}s)" -ForegroundColor Red
        $report += "- ❌ **JAR**: Build FAILED (${jarDuration}s)`n`n"
        $jarFailed++
        $failedServices += $service
    }
...
if ($compileFailed -eq 0 -and $jarFailed -eq 0) {
    ...
    "**Next Steps**:`n"
    "1. ⚠️ **Test Coverage Verification**: Run separately (requires JaCoCo setup)`n"
    "2. ⚠️ **Smoke Tests**: Run in staging environment`n"
    "3. ✅ **Ready for deployment**: All services can be packaged and deployed`n"
} elseif ($compileFailed -eq 0 -and $jarFailed -gt 0) {
    "### ⚠️ PARTIAL BUILD SUCCESS`n`n"
    "✅ **Compilation**: All $($services.Count) services compiled successfully`n"
    "⚠️ **JAR Build**: $jarFailed services failed to build JARs`n"
    ...
    "### ❌ BUILD FAILURES DETECTED`n`n"
    "❌ **Compilation**: $compileFailed services failed to compile`n"
    "⚠️ **JAR Build**: $jarFailed services failed to build JARs`n"
    "✅ **Successful Builds**: $jarSuccess/$($services.Count)`n`n"
...
if ($compileFailed -eq 0 -and $jarFailed -eq 0) {
    ...
    Write-Host "⚠️ PARTIAL SUCCESS: All compiled, some JAR builds failed" -ForegroundColor Yellow
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "DEPLOYMENT CRITICAL PATH: FAILED ❌" -ForegroundColor Red
}
