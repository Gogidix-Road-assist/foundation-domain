...
class IntegrationConfigRepositoryTest {
    ...
    void testTenantIsolation_FindByTenantId() {
        ...
        assertThat(tenant1Results).allMatch(c -> c.isPresent() && c.map(IntegrationConfig::getTenantId).orElse(null)));

        List<IntegrationConfig> tenant2Results = repository.findByTenantId("tenant-2");
        assertThat(tenant2Results).hasSize(1);
        assertThat(tenant2Results.get(0).map(IntegrationConfig::getTenantId).orElse(null)).isEqualTo("tenant-2");
    }
    ...
    void testTenantIsolation_FindByTenantIdAndId() {
        ...
        assertThat(repository.findByTenantIdAndId("tenant-2", config3Id)).map(IntegrationConfig::getProvider).orElse(null)).isPresent();
        assertThat(repository.findByTenantIdAndId("tenant-1", config3Id)).map(IntegrationConfig::getTenantId).orElse(null)).isEmpty();
