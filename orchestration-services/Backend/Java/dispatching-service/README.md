...
The Dispatching Service is responsible for:
...
### Access API
- Service: http://localhost:8084/api/v1
...
### Build Image
```bash
docker build -t dispatching-service .
```

### Run Container
```bash
docker run -p 8084:8084 \
  -e SPRING_DATA_MONGODB_URI=mongodb://host.docker.internal:27017/orchestration_dispatching_service_db \
  dispatching-service
```
