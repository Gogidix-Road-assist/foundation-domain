package com.gogidix.rapidassist.tenancy.configuration.service.infrastructure.adapter.rest;
...
/**
 * REST API client for communicating with the Config Service.
 *
 * <p>This adapter allows the tenant configuration service to validate
 * tenant IDs and fetch configuration from the central config service.
 */
@Component
public class ConfigServiceApiClient {

    private static final Logger logger = LoggerFactory.getLogger(ConfigServiceApiClient.class);

    private final RestTemplate restTemplate;
    private final String configServiceBaseUrl;
    private final String apiKey;

    public ConfigServiceApiClient(
            RestTemplateBuilder restTemplateBuilder,
            @Value("${app.config-service.base-url:http://config-service:8080}") String baseUrl,
            @Value("${app.config-service.api-key:}") String apiKey) {
        this.configServiceBaseUrl = baseUrl;
        this.apiKey = apiKey;
    ...
    public boolean isTenantActive(String tenantId) {
        try {
            String url = configServiceBaseUrl + "/api/v1/tenants/" + tenantId + "/status";
    ...
    public Optional<TenantConfiguration> getTenantConfiguration(String tenantId) {
        try {
            String url = configServiceBaseUrl + "/api/v1/tenants/" + tenantId;
    ...
    /**
     * Records a tenant configuration access for auditing.
     *
     * @param tenantId    the tenant ID
     * @param configKey   the configuration key accessed
     * @param accessedBy  the user who accessed it
     */
    public void recordAccess(String tenantId, String configKey, String accessedBy) {
        try {
            String url = configServiceBaseUrl + "/api/v1/tenants/" + tenantId + "/audit";
            ...
            Map<String, String> auditEntry = Map.of(
                "action", "TENANT_CONFIG_ACCESS",
                "resource", "tenant-config:" + configKey,
                "userId", accessedBy
            );
