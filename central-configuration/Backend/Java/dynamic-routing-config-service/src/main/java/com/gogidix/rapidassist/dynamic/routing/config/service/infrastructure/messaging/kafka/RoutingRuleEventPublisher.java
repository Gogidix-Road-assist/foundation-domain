...
/**
 * Kafka event publisher for routing rule domain events.
 *
...
public class RoutingRuleEventPublisher {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RoutingRuleEventPublisher.class);
    ...
    @Autowired
    public RoutingRuleEventPublisher(
            KafkaTemplate<String, Object> kafkaTemplate,
            @Value("${app.kafka.topics.events:routing-rule.events}") String eventsTopic) {
    ...
    /**
     * Publishes a routing rule created event.
     *
     * @param event the event to publish
     * @return CompletableFuture that completes when published
     */
    @Transactional
    public CompletableFuture<Void> publishRoutingRuleCreated(RoutingRuleCreatedEvent event) {
        return publishEvent(event, "RoutingRuleCreated");
    }

    /**
     * Publishes a routing rule updated event.
     *
     * @param event the event to publish
     * @return CompletableFuture that completes when published
     */
    @Transactional
    public CompletableFuture<Void> publishRoutingRuleUpdated(RoutingRuleUpdatedEvent event) {
        return publishEvent(event, "RoutingRuleUpdated");
    }

    /**
     * Publishes a routing rule deleted event.
     *
     * @param event the event to publish
     * @return CompletableFuture that completes when published
     */
    @Transactional
    public CompletableFuture<Void> publishRoutingRuleDeleted(RoutingRuleDeletedEvent event) {
        return publishEvent(event, "RoutingRuleDeleted");
    }
