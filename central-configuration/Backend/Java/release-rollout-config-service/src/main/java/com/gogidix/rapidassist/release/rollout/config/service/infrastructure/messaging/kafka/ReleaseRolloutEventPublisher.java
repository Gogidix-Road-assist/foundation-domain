package com.gogidix.rapidassist.release.rollout.config.service.infrastructure.messaging.kafka;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.databind.json.JsonMapper;
import com.gogidix.rapidassist.release.rollout.config.service.domain.event.DomainEvent;
...
/**
 * Kafka event publisher for release rollout domain events.
 *
...
public class ReleaseRolloutEventPublisher {

    private static final Logger logger = LoggerFactory.getLogger(ReleaseRolloutEventPublisher.class);
    ...
    @Autowired
    public ReleaseRolloutEventPublisher(
            KafkaTemplate<String, Object> kafkaTemplate,
            @Value("${app.kafka.topics.events:release-rollout.events}") String eventsTopic) {
    ...
    /**
     * Publishes a rollout created event.
     *
     * @param event the event to publish
     * @return CompletableFuture that completes when published
     */
    @Transactional
    public CompletableFuture<Void> publishReleaseRolloutCreated(ReleaseRolloutCreatedEvent event) {
        return publishEvent(event, "ReleaseRolloutCreated");
    }

    /**
     * Publishes a rollout updated event.
     *
     * @param event the event to publish
     * @return CompletableFuture that completes when published
     */
    @Transactional
    public CompletableFuture<Void> publishReleaseRolloutUpdated(ReleaseRolloutUpdatedEvent event) {
        return publishEvent(event, "ReleaseRolloutUpdated");
    }

    /**
     * Publishes a rollout deleted event.
     *
     * @param event the event to publish
     * @return CompletableFuture that completes when published
     */
    @Transactional
    public CompletableFuture<Void> publishReleaseRolloutDeleted(ReleaseRolloutDeletedEvent event) {
        return publishEvent(event, "ReleaseRolloutDeleted");
    }
